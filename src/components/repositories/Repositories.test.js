...
describe('Repositores', () => {
    ...
    // const respositories = render(<Repositories/>);
    // respositories.debug();
    ...
    it('list the repositories', async () => {
        let respositories = [];
        act(() => {
            respositories = await render(<Repositories/>);
          });
          /* assert on the output */
        respositories.debug();
        // const {debug} = render(<Repositories/>);
        // debug();
        // expect(screen.getByText('BabelNetPoc')).toBeInTheDocument();
